...
public class FollowersTest {
    ...
    public void testFollowersButton() {
        ...
        onView(withId(R.id.buttonFollowers)).perform(ViewActions.click());

        // Verifica que la nueva actividad de seguidores se muestra correctamente
        onView(withId(R.id.recyclerViewFollowers)).check(ViewAssertions.matches(ViewMatchers.isDisplayed()));

        // Verifica que el numero de seguidores es correcto
        onView(withId(R.id.recyclerViewFollowers)).check(new RecyclerViewItemCountAssertion(3));
